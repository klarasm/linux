		...
		bool *mmap_locked, unsigned long enabled_orders)
{
	...
	DECLARE_BITMAP(mask, HPAGE_PMD_NR);
	...
	while (top >= 0) {
		...
		bitmap_zero(mask, HPAGE_PMD_NR);
		bitmap_set(mask, offset, num_chunks);
		bits_set = bitmap_weight_and(cc->mthp_bitmap, mask, HPAGE_PMD_NR);
...
out_unmap:
	...
	if (result == SCAN_SUCCEED) {
		result = collapse_scan_bitmap(mm, address, referenced, unmapped,
