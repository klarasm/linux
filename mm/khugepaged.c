...
static unsigned int mthp_nr_occupied_pte_entries(struct collapse_control *cc, u16 offset, unsigned long nr_pte_entries)
{
 ...
 * stack at a lower order to represent the left and right halves of the PTE
 * page table we are examining.
 *
...
retry:
		...
		if (result == SCAN_PAGE_DIRTY_OR_WRITEBACK && !retried) {
			...
			if (!mmap_locked) {
				...
				mmap_read_lock(mm);
				mmap_locked = true;
				result = hugepage_vma_revalidate(
					mm, addr, false, &vma, cc,
					HPAGE_PMD_ORDER);
				if (result != SCAN_SUCCEED)
					goto handle_result;
			}
