...
static bool reboot = CONFIG_SPACEMIT_K1_WATCHDOG_REBOOT;
module_param(reboot, bool, 0);
...
static int k1_wdt_probe(struct platform_device *pdev)
{
	...
	data->clk = devm_clk_get_enabled(dev, "core");
	if (IS_ERR(data->clk))
		return dev_err_probe(dev, PTR_ERR(data->clk),
				"failed to get watchdog clock\n");

	data->bus_clk = devm_clk_get_enabled(dev, "bus");
	if (IS_ERR(data->bus_clk))
		return dev_err_probe(dev, PTR_ERR(data->bus_clk),
